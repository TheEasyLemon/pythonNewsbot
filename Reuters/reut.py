...
def main():
    ...
    def daterange(start_date, end_date):
        """(date object, date object) => generator object of date objects

        daterange produces a generator object using the timedelta method from datetime.
        
        """
        for n in range(int((end_date - start_date).days)):
            yield start_date + timedelta(n)

    # start date is last_opened
    start_date = date(last_opened[0], last_opened[1], last_opened[2])
    # end date is today
    end_date = date.today()

    # for each date in between, get that day's sitemap
    for d in daterange(start_date, end_date):
        day1 = f"{d.year}{d.month:02d}{d.day:02d}"
        day2 = f"{d.year}{d.month:02d}{d.day + 1:02d}"
        xmlurl = f"https://www.reuters.com/sitemap_{day1}-{day2}.xml"
        
        urlmatches = ["article"]
        
        # get list of article_urls from each sitemap
