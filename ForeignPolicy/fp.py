...
for i in range(int(last_opened[1]), now.month):
    for j in range(int(last_opened[2]), days_in_month[i]):
        xml_maps.append("https://foreignpolicy.com/sitemap.xml?yyyy=" +
                        str(now.year) + "&mm=" + add_zeroes(i) + "&dd=0" +
                        add_zeroes(j))
        # https://foreignpolicy.com/sitemap.xml?yyyy=(year)&mm=(month)&dd=(day)

# this month
for k in range(int(last_opened[2]), now.day):
    xml_maps.append("https://foreignpolicy.com/sitemap.xml?yyyy=" +
                        str(now.year) + "&mm=" + add_zeroes(now.month) + "&dd=0" +
                        add_zeroes(k))
    # https://foreignpolicy.com/sitemap.xml?yyyy=(year)&mm=(month)&dd=(day)
...
for url in xml_maps:
    ...
    for article_url in sitemap_urls:
        ...
        path = ("/Users/dawsonren/Documents/pythonNewsbot/News/FP/" +
                format_title + ".txt")
        ...
        try:
            with open(path, "w") as file:
                ...
                file.write("Source: Foreign Policy")
